...
var DirhamSymbolBase = forwardRef(
  ({
    size = 24,
    color = "currentColor",
...
var DirhamSymbol = memo(DirhamSymbolBase);
DirhamSymbol.displayName = "DirhamSymbol";
var DirhamIconBase = forwardRef(
  ({
    size,
    color = "currentColor",
    "aria-label": ariaLabel = "UAE Dirham",
...
var DirhamIcon = memo(DirhamIconBase);
  ...
  ({
    ...
    return /* @__PURE__ */ jsx(
      "span",
      {
        ref,
        className,
        dir: locale.startsWith("ar") ? "rtl" : void 0,
...
function formatInputValue(value, locale, decimals) {
...
var DirhamInputBase = forwardRef(
  ({
    ...
    const displayText = isFocused ? rawText : formatInputValue(numericValue, locale, decimals);
    const handleFocus = useCallback(
      (e) => {
        setIsFocused(true);
        const plain = numericValue !== void 0 && Number.isFinite(numericValue) ? numericValue.toString() : "";
        setRawText(plain);
        if (selectOnFocus) {
    ...
    const symbolElement = showSymbol ? useCode ? /* @__PURE__ */ jsx(
      "span",
      {
        style: {
          userSelect: "none",
...
var DirhamInput = memo(DirhamInputBase);
...
var AnimatedDirhamPriceBase = forwardRef(
  ({
    amount,
    duration = 600,
    easing = "easeOut",
    ...
    const symbolFirst = !locale.startsWith("ar");
    const isRTL = locale.startsWith("ar");
    const formatted = Number.isFinite(displayAmount) ? formatDirham(displayAmount, {
      locale,
      decimals,
      useCode: true,
      notation
...
var AnimatedDirhamPrice = memo(AnimatedDirhamPriceBase);
...
// src/react/useDirhamRate.ts
function useDirhamRate(currency) {
  const [rate, setRate] = useState(void 0);
  ...
  const convertBack = useCallback(
    (amount) => {
      if (rate === void 0 || rate === 0) return void 0;
      const factor = 100;
      return Math.round(amount / rate * factor) / factor;
    },
    [rate]
...
export { AnimatedDirhamPrice, DirhamIcon, DirhamInput, DirhamPrice, DirhamSymbol, useDirhamRate };
